...
import gym
from network_ppo import network_factory
from network_ppo import PolicyNetwork
from network_ppo import ValueNetwork
...
def lambda_returns(rewards, lambda_value_estimates, gamma = 1, l=0.95):
    r = rewards[::-1]
    V = lambda_value_estimates.view(1,-1)[0].tolist()[::-1]
    G = [r[0]]
    
    for i in range(1,len(r)):
        G.append(r[i] + gamma* ((1 - l)*V[i-1] + l*G[i-1]) )
    G = G[::-1]
    return G
...
def reinforce(env, policy_estimator, value_estimator, num_episodes, # value_estimator=None,
    ...
    # Define optimizer
    optimizer =   optim.Adam(policy_estimator.network.parameters(),  lr=0.0005)
    optimizer_v = optim.Adam(value_estimator.network_v.parameters(), lr=0.0005)
    ...
    for ep in range(num_episodes):
        ...
        while complete == False:
            ...
            if complete:
                ...
                batch_rewards.extend(lambda_returns(rewards, lambda_value_estimates, gamma=1, l=0.95))
                ...
                if batch_counter == batch_size:
                    if flag == 1:
                        ...
                        for epoch in range(10):
                            ...
                            for i in range(1):
                                miniindices = indices[int(1.0*i*len(batch_rewards)/1):int(1.0*(i+1)*len(batch_rewards)/1)]
                                ...
                                final_normalized_clipping = torch.min(r.view(1,-1)[0] * normalized_advantage, clipped_r.view(1,-1)[0] * normalized_advantage)
                                
                                loss = -torch.mean(final_normalized_clipping) + loss_v + torch.mean( - 0.01 * Categorical(policy_estimator.forward(state_tensor)).entropy())
...
if __name__ == '__main__':
    ...
    for run in range(numrun):
        ...
        fwrite = open('runs_data_cartpole1_30runs/ppo/'+str(run)+'.pkl','wb')
