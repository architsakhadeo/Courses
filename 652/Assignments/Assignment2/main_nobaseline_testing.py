...
if __name__ == '__main__':
    ...
    for policy in range(numpolicy):
        for run in range(numrun):
            ...
            # Load policy to test
            pe.network.load_state_dict(torch.load('saved_policies_cartpole1_1run_new/training/nobaseline/saved_network'+str(policy)+'.pkl'))
            ve = ValueNetwork(in_size)
            ep_returns = reinforce(env, pe, ve, episodes, batch_size, gamma) #,ve , loss_policy, loss_value
                
            fwrite = open('runs_data_cartpole1_50runs_new/testing/nobaseline/'+str(5*policy+run)+'.pkl','wb')
            pickle.dump(ep_returns, fwrite)
            fwrite.close()
            #torch.save(pe.network.state_dict(), 'saved_policies_cartpole1_1run_new/training/nobaseline/saved_network'+str(run)+'.pkl')            
