            ...
            if complete:
                ...
                if batch_counter == batch_size:
                    if flag == 1:
                        ...
                        #loss = - torch.mean(policy_estimator.forward(state_tensor).log_prob(action_tensor)*(reward_tensor-value_estimates.view(1,-1)[0])) #-value_estimates
                        loss = - (gamma**t) * torch.mean(policy_estimator.forward(state_tensor).log_prob(action_tensor)*(reward_tensor))
                        optimizer.zero_grad()
                ...
                #if (ep + 1) % 10000 == 0:
                #    torch.save(pe.network.state_dict(), 'saved_policies_cartpole1_30runs/nobaseline/saved_network_'+ str(ep + 1) + '.pkl')
                '''
                if flag == 1:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--episodes", "-e", default=100, type=int, help="Number of episodes to train for")
    ...
    for policy in range(numpolicy):
        for run in range(numrun):
            ...
            # Load policy to test
            pe.network.load_state_dict(torch.load('saved_policies_cartpole1_1run_new/training/nobaseline_discounted/saved_network'+str(policy)+'.pkl'))
            ve = ValueNetwork(in_size)
            ep_returns = reinforce(env, pe, ve, episodes, batch_size, gamma) #,ve , loss_policy, loss_value
                
            fwrite = open('runs_data_cartpole1_50runs_new/testing/nobaseline_discounted/'+str(5*policy+run)+'.pkl','wb')
            pickle.dump(ep_returns, fwrite)
            fwrite.close()
            #torch.save(pe.network.state_dict(), 'saved_policies_cartpole1_1run_new/training/nobaseline_discounted/saved_network'+str(run)+'.pkl')
